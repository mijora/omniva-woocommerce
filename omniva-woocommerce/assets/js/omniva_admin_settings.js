...
jQuery('document').ready(function($){
  ...
  var omnivalt_settings = {
    get_api_country: function() {
      return document.getElementById("woocommerce_omnivalt_api_country").value;
    },

    get_available_methods: function( api_country = null ) {
      if ( ! api_country ) {
        api_country = omnivalt_settings.get_api_country();
      }

      return omnivalt_params.available_methods[api_country];
    },

    get_available_prices_blocks: function() {
      let country, i, prices_block;
      let available_methods = omnivalt_settings.get_available_methods();
      let available_blocks = {};

      for ( country in available_methods ) {
        available_blocks[country] = [];
        for ( i = 0; i < available_methods[country].length; i++ ) {
          prices_block = $(".prices_box[data-country='" + country + "']").find(".block-prices." + available_methods[country][i])[0];
          available_blocks[country].push(prices_block);
        }
      }
    ...
    get_unique_available_methods: function() {
      let country, i;
      let available_methods = omnivalt_settings.get_available_methods();
      let unique_methods = [];
      ...
      for ( country in available_methods ) {
        for ( i = 0; i < available_methods[country].length; i++ ) {
          if ( unique_methods.indexOf(available_methods[country][i]) === -1 ) {
            unique_methods.push(available_methods[country][i]);
          }
        }
      }
    ...
    enable_api_country_methods: function() {
      ...
      for ( country in available_blocks ) {
        for ( i = 0; i < available_blocks[country].length; i++ ) {
          $(available_blocks[country][i]).removeClass('disabled');
        }
      }
    },

    get_all_shipping_methods_cb: function() {
      return $('input[id^="woocommerce_omnivalt_method_"]');
    },

    refresh_cb_by_api_country: function() {
      let i, method;
      let unique_methods = omnivalt_settings.get_unique_available_methods();
      let all_cb = omnivalt_settings.get_all_shipping_methods_cb();

      for ( i = 0; i < all_cb.length; i++ ) {
        method = all_cb[i].dataset.method;
        if ( method == "terminal" ) method = "pickup";

        if ( unique_methods.indexOf(method) === -1 ) {
    ...
    toggle_prices_blocks_by_cb: function( checkbox ) {
      let country, i;
      let available_blocks = omnivalt_settings.get_available_prices_blocks();
      let checkbox_method = checkbox.dataset.method;

      $(".block-prices." + checkbox_method).addClass('disabled');

      for ( country in available_blocks ) {
        for ( i = 0; i < available_blocks[country].length; i++ ) {
          if ( ! $(available_blocks[country][i]).hasClass(checkbox_method) ) {
            continue;
          }
          ...
          if ( checkbox.checked ) {
            ...
            $(available_blocks[country][i]).addClass('disabled');
          }
        }
      }
    }
