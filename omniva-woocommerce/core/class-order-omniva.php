...
class OmnivaLt_Order_Omniva
{
        ...
        foreach ( $order_methods_list as $ship_method ) {
            foreach ( $configs['method_params'] as $method_name => $method_values ) {
                ...
                if ( $ship_method == "omnivalt_" . $method_values['key'] ) {
                    OmnivaLt_Order_WC::update_meta($order_id, $configs['meta_keys']['method'], $ship_method);
                    return true;
                }
            }
        }
    ...
    public static function get_method( $order_id )
    {
        ...
        $send_methods = OmnivaLt_Order_WC::get_shipping_methods($order_id);
        
        foreach ( $send_methods as $method ) {
            if ( $method == 'omnivalt' ) {
                return OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['method']);
            }
        }
    ...
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::update_meta($order_id, $meta_keys['terminal_id'], $terminal_id);
    }

    public static function get_terminal_id( $order_id )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['terminal_id']);
    }
    ...
    public static function set_barcodes( $order_id, $barcodes )
    {
        ...
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::update_meta($order_id, $meta_keys['barcodes'], $barcodes);
    }

    public static function get_barcodes( $order_id )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        $barcodes = OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['barcodes']);
    ...
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::update_meta($order_id, $meta_keys['error'], $error_msg);
    }

    public static function get_error( $order_id )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['error']);
    }

    public static function set_dimmensions( $order_id, $dimmensions )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::update_meta($order_id, $meta_keys['dimmensions'], $dimmensions);
    }

    public static function get_dimmensions( $order_id )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['dimmensions']);
    }

    public static function set_manifest_date( $order_id, $manifest_date )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::update_meta($order_id, $meta_keys['manifest_date'], $manifest_date);
    }

    public static function get_manifest_date( $order_id )
    {
        $meta_keys = OmnivaLt_Core::get_configs('meta_keys');
        return OmnivaLt_Order_WC::get_meta($order_id, $meta_keys['manifest_date']);
    }
}
